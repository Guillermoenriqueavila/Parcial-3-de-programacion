...
namespace Parcial_III
{
    public partial class Modificar : System.Web.UI.Page
    {
        ...
        ListItem i;
        ListItem carreras;
        string ddList, ddListLoad;
        string ddCarrer, ddCarrerLoad;
        ListItem rangos;
        int load = 0;
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (load == 0){
                ...
                cmd.CommandText = "select id, código, nombre, usuario, contraseña, rango, telefono, correo, facultad, carrera from estudiantes where id = " + valor;
                cmd.ExecuteNonQuery();
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    txtCodigo.Text = dr["código"].ToString();
                    txtCorreo.Text = dr["correo"].ToString();
                    txtTelefono.Text = dr["telefono"].ToString();
                    txtContrasenna.Text = dr["contraseña"].ToString();
                    ddRango.Text = dr["rango"].ToString();
                    txtUsuario.Text = dr["usuario"].ToString();
                    ddListLoad = dr["facultad"].ToString();
                    ddCarrerLoad = dr["carrera"].ToString();


                }
                con.Close();
                ddListaCargar();
                ddLista.SelectedValue = ddCarrerLoad;
                ddList = ddCarrerLoad;
                ddListaCargar();
                ddCarreraCargar();
                ddCarrera.SelectedValue = ddCarrerLoad;
            }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            cmd2.CommandText = "update estudiantes set código='" + txtCodigo.Text + "', nombre = '" + txtNombre.Text + "', usuario = '" + txtUsuario.Text + "', contraseña = '" + txtContrasenna.Text + "', rango='" + ddRango.Text + "', telefono='" + txtTelefono.Text + "', correo='" + txtCorreo.Text + "', facultad='" + ddLista.Text + "', carrera='" + ddCarrera.Text + "' where id=" + idoculto.Text;
